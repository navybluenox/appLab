...
function loadTextFileFromDrive(fileId, charEnc) {
    if (charEnc === void 0) { charEnc = "UTF-8"; }
...
function outputLogs(comments, option) {
    if (option === void 0) { option = { "inputTime": true }; }
    ...
    if (sheetSize.height === -1)
        sheetSize.height = allRangeValues.length;
    if (sheetSize.width === -1)
        sheetSize.width = allRangeValues[0].length;
...
function dateString(date, template) {
    ...
    var fillD = function (value, fillStr, digit) {
